...
export const TOWERS = {
  willow: {
    name: 'Willow Warden', key: '1', cost: 100, range: 9, rate: 1.6, dmg: 8, color: 0x9fce6a,
    desc: 'Swift longbow watch arcing leaf-arrows at a single foe.',
    lv: [
      { cost: 75, dmg: 8, rate: 1.6, range: 9 },
      { cost: 95, dmg: 14, rate: 1.5, range: 10 },
      { cost: 130, dmg: 24, rate: 1.35, range: 11.5 }
    ]
  },
  forge: {
    name: 'Ember Forge', key: '2', cost: 140, range: 11, rate: 0.62, dmg: 26, color: 0xe08a3c,
    desc: 'Lobs blazing caltrops that burst for area damage.',
    lv: [
      { cost: 140, dmg: 26, rate: 0.62, range: 11, splash: 2.6 },
      { cost: 110, dmg: 42, rate: 0.60, range: 12, splash: 3.1 },
      { cost: 150, dmg: 66, rate: 0.56, range: 13, splash: 3.7 }
    ]
  },
  frost: {
    name: 'Frostbloom Totem', key: '3', cost: 120, range: 7.5, rate: 1.0, dmg: 5, color: 0x8fd8e8,
    desc: 'Sighing petals chill all in range; slow 45% and nip HP.',
    lv: [
      { cost: 120, dmg: 5, rate: 1.0, range: 7.5, slow: 0.45 },
      { cost: 100, dmg: 9, rate: 1.0, range: 8.2, slow: 0.52 },
      { cost: 140, dmg: 15, rate: 1.0, range: 9.0, slow: 0.60 }
    ]
  },
  storm: {
    name: 'Storm Drum', key: '4', cost: 160, range: 10, rate: 0.8, dmg: 18, color: 0xc9a3ff,
    desc: 'A cracked drum that sings chain lightning across 3 foes.',
    lv: [
      { cost: 160, dmg: 18, rate: 0.8, range: 10, chain: 3 },
      { cost: 120, dmg: 28, rate: 0.78, range: 10.5, chain: 3 },
      { cost: 165, dmg: 44, rate: 0.74, range: 11.5, chain: 4 }
    ]
  },
  lumen: {
    name: 'Lumen Hearth', key: '5', cost: 200, range: 8.5, rate: 0.45, dmg: 40, color: 0xffd98a,
    desc: 'A lantern heart thrumming a radiant blast at its core.',
    lv: [
      { cost: 200, dmg: 40, rate: 0.45, range: 8.5 },
      { cost: 150, dmg: 62, rate: 0.45, range: 9.0 },
      { cost: 200, dmg: 95, rate: 0.44, range: 9.5 }
    ]
  }
};

export const ENEMIES = {
  wisp:     { name: 'Flitter Wisp',    hp: 46,  speed: 3.4, reward: 12, color: 0xd7f27e, kind: 'fly' },
  beetle:   { name: 'Crowned Beetle',  hp: 150, speed: 1.5, reward: 22, color: 0x6f9d55, kind: 'ground' },
  ember:    { name: 'Emberling',       hp: 78,  speed: 2.5, reward: 16, color: 0xf29c55, kind: 'ground' },
  stalker:  { name: 'Briar Stalker',   hp: 95,  speed: 2.6, reward: 18, color: 0xb7c49b, kind: 'ground', jitter: 1.6 },
  grunt:    { name: 'Mossback Grunt',  hp: 210, speed: 1.9, reward: 28, color: 0x87956a, kind: 'ground' },
  boss:     { name: 'Stonehorn Sentinel', hp: 3200, speed: 0.95, reward: 300, color: 0x9aa79b, kind: 'boss' }
};
...
export const WAVE_BONUS = { 1: 60, 2: 90, 3: 40 };
...
function hurt(s, e, dmg, opts = {}) {
  if (e.dead) return;
  const mult = opts.armor ? 1 - Math.min(0.55, e.armor || 0) : 1;
  e.hp -= dmg * mult;
  e.flash = 0.12;
  if (opts.slow && e.slowT < opts.slow) { e.slowT = opts.slow; e.slowD = opts.slowDur || 2.2; }
  if (e.hp <= 0) {
    e.dead = true;
    s.kills++;
    s.gold += e.reward;
...
export function step(s, dtRaw) {
  ...
  for (const e of s.enemies) {
    if (e.dead) continue;
    let sp = e.speed;
    if (e.slowT > 0) { sp *= (1 - e.slowT); e.slowD -= dt; if (e.slowD <= 0) e.slowT = 0; }
    e.d += sp * dt;
    ...
    if (e.d >= s.path.len - 0.6) {
      ...
      s.leaks++;
      s.lives--;
      s.events.push({ type: 'leak', id: e.id, boss: e.kind === 'boss' });
  ...
  for (const t of s.towers) {
    if (t.cd > 0) t.cd -= dt;
    // find target: nearest-to-heath in range
  ...
  for (const p of s.projectiles) {
    ...
    const tp = p.targets;
    if (k >= 1) {
      for (const te of tp) if (!te.dead) hurt(s, te, p.dmg, { slow: p.slow, slowDur: 2.2 });
      if (p.splash) {
        for (const e of s.enemies) {
          if (e.dead) continue;
          const d2 = dist2(p.tx, p.tz, e.x, e.z);
          if (d2 <= p.splash * p.splash) hurt(s, e, p.splashDmg || p.dmg * 0.5);
        }
  ...
  for (const o of s.orbs) {
    ...
    if (k >= 1) {
      for (const e of s.enemies) {
        if (e.dead) continue;
        if (dist2(o.x, o.z, e.x, e.z) <= o.r * o.r) hurt(s, e, o.dmg);
      }
...
function fireTower(s, t, target) {
  ...
  if (type === 'willow') {
    ...
    s.projectiles.push({ kind: 'ember', t: 0, dur: 0.5 + d * 0.075, targets: [target], tx: target.x, tz: target.z, sx: t.x, sz: t.z, dmg: t.dmg, splash: t.splash, splashDmg: t.dmg });
  } else if (type === 'frost') {
    // spiral of petals around the totem hitting all in range
    for (const e of s.enemies) {
      if (e.dead) continue;
    ...
    for (const te of chain) hurt(s, te, t.dmg, {});
  } else if (type === 'lumen') {
